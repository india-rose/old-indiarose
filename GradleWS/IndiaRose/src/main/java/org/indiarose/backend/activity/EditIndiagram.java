	...
	protected boolean saveIndiagram()
	{
		...
		else
		{
			...
			if(this.m_originalIndiagram instanceof Category)
			{
						...
						Indiagram n = new Indiagram(m_indiagramText.getText().toString(), m_indiagram.imagePath, m_indiagram.soundPath);
						n.filePath = m_indiagram.filePath;
						m_parent.indiagrams.add(n);
						m_parent.indiagrams.remove(m_indiagram);
						File f = new File(PathData.XML_DIRECTORY + m_indiagram.filePath);
						f.delete();
						File d = new File(PathData.XML_DIRECTORY + ((Category)m_indiagram).filePath.substring(0, ((Category)m_indiagram).filePath.length() - ".xml".length()));
						d.delete();
						
						AppData.current_indiagram = n;
			...
			else
			{
				...
				c.filePath = this.m_indiagram.filePath;
				this.m_parent.indiagrams.add(c);
				
				//Suppression of the indiagram
				m_parent.indiagrams.remove(m_indiagram);
				File f = new File(PathData.XML_DIRECTORY + m_indiagram.filePath);
				f.delete();
				m_indiagram.filePath="";
				
				AppData.current_indiagram = c;
