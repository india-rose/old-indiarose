 ...
 */
public class CategoryBrowser extends AbstractScreen {
	...
	public CategoryBrowser(ScreenManager _screen) {
		super(_screen);
		...
		ButtonItem[] buttons = new ButtonItem[] { backButton, nextButton };
	...
	protected void sendSignal(Indiagram _indiagram) {
		try {
			Mapper.emit(this, "selected", _indiagram);
		} catch (MapperException e) {
			Log.wtf("CategoryBrowser", "Error while emitting signal", e);
		}
	}
