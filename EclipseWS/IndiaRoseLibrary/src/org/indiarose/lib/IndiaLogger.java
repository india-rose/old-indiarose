...
public class IndiaLogger {
	...
	public static void writeCorrections(List<Indiagram> lastList,
			List<Indiagram> afterList) {

	}

	public static void addActionLog(Context currentContext, String description,
			String type, long timestamp) {
		AccountManager manager = (AccountManager) currentContext
	...
	private static void control(ActionLog a) {
		if (lesActions.size() > seuilFlush
				&& a.getType().equals(ActionLog.TYPE_VALIDER)) {
			write();
	...
	private static String findNextFile() {
		...
		if (f.list().length > 0) {
			...
			for (String s : lesFiles) {
				...
				String rank = s.substring(s.lastIndexOf("_") + 1,
						s.lastIndexOf(FILEEXTENSION));
			...
			res++; // on veux Ã©crire dnas un nouveau fichier donc on fais
					// monter le rank de 1.
	...
	/**
	 * dans cette mÃ©thode on cherche la derniÃ¨re balise valider pour
	 * sauvegarder le plus de phrases non flush et ensuite on les Ã©crit dans le
	 * fichier Ã  l'aide de write
	 */
	public static void flushB4Save() {
		...
		if (nbActions != 0) {
			...
			while (!found && index >= 1) {
				...
				if (lesActions.get(index).getType()
						.equals(ActionLog.TYPE_VALIDER)) { // si l'action est un
															// type valider,
															// c'est la fin
															// d'une phrase
					found = true;
			...
			if (found) {
				...
				// on definit lesActions Ã  sauvegardÃ© avec la nouvelle liste
				// construite
				lesActions = res;
				write();
				removeAllAction();
	...
	private static final long tailleMaxLogs = 1024 * 1024;// la taille d'1Mo en
															// bytes

	public static void rmLogsIfTooHuge() {
		System.out.println("indialogger - directory size : "
				+ dirSize(new File(DIRPATH)));
	...
	public void sendLogs(Activity act) {
		...
		builder.setMessage(act.getResources().getString(
				R.string.logAlertMessage));
		builder.setTitle(act.getResources().getString(R.string.logAlertTitle));
		builder.setPositiveButton(act.getResources().getString(R.string.ok),
		...
		builder.setNegativeButton(a.getResources().getString(R.string.cancel),
				new DialogInterface.OnClickListener() {
	...
	public void sendlogV2(Activity act) {
		...
		mDialog.setMessage(a.getResources().getString(
				R.string.logProgressMessage));
		...
		if (f != null && f.exists()) {
			...
			if (fileCount > 0) {
				...
				mDialog.setButton(
						a.getResources().getString(R.string.stopSending),
						new DialogInterface.OnClickListener() {
							@Override
		...
		act.runOnUiThread(new Runnable() {
			public void run() {
				Toast.makeText(a,
						a.getResources().getString(R.string.logSended),
	...
	class NetLoggerAsyncTaskV2 extends AsyncTask<Activity, Void, Void> {
		...
		protected Void doInBackground(Activity... params) {
					...
					if (networkInfo != null && networkInfo.isConnected()) 
					{
						...
						for (String currentFile : f.list()) {
							...
							if (f.list().length != 0) {
								...
								ObjectMapper mapper = new ObjectMapper();
								try {
									ArrayList<ActionLog> lesAct = mapper.readValue(
													logfile,
													new TypeReference<ArrayList<ActionLog>>() {
													});

									boolean sendResult = NetLogger
											.envoyer(lesAct, this.act);

									if (sendResult) {
										
										DeleteRecursive(logfile);
										current++;
										...
										this.act.runOnUiThread(new Runnable() {
											public void run() {
												...
												Toast.makeText(
														a,
														"An error happened with server result",
														Toast.LENGTH_LONG)
														.show();
											}
										});
			...
			act.runOnUiThread(new Runnable() {
				public void run()
				{
					Toast.makeText(a, a.getResources().getString(R.string.logSended), Toast.LENGTH_SHORT).show();
				}
			});
			return null;
		}
